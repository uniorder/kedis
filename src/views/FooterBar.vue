<template>
  <div class="main-foot d-flex flex-row">
    <div class="br px-2" v-if="server.useSSH">SSH</div>
    <div class="br px-2" v-if="server.host && !server.useSSH">
      {{ server.host }}
    </div>
    <div class="br px-2" v-if="server.useSSH">
      {{ server.sshHost }}
    </div>
    <div class="br px-2" v-if="server.info.CPU">
      <i class="fas fa-microchip"></i>
      {{ (server.info.CPU.used_cpu_sys * 1).toFixed(2) }} %
    </div>
    <div class="br px-2" v-if="server.info.CPU">
      <i class="fas fa-memory"></i>
      {{ (server.info.Memory.used_memory / 1024 / 1024).toFixed(2) }}mb
    </div>
    <div class="br px-2" v-if="server.info.CPU">
      <i class="fab fa-linux" v-if="checkOs() === 'Linux'"></i>
      <i class="fab fa-apple" v-if="checkOs() === 'Darwin'"></i>
      <i class="fab fa-windows" v-if="checkOs() === 'Windows'"></i>
      {{ server.info.Server.os }}
    </div>
    <div class="br px-2" v-if="server.info.CPU">
      <svg
        width="16"
...
export default class FootBar extends Vue {
  ...
  public showServerInfo(server: RedisServer): void {
    this.server = server;
  }

  private checkOs(): string {
    if (this.server.info.Server.os.includes("Linux")) {
      return "Linux";
    } else if (this.server.info.Server.os.includes("Windows")) {
      return "Windows";
    } else if (this.server.info.Server.os.includes("Darwin")) {
      return "Darwin";
    } else {
      return "";
    }
