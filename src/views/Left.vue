<template>
  <div class="main-left" style="overflow-y:auto;" @contextmenu="showPanelContextMenu">
    <tree ref="tree" v-model="nodes" @toggle="loadData" @nodeclick="emitNodeClick" @nodecontextmenu="showServerContextMenu">
      ...
      <template slot="sidebar" slot-scope="{ node }">
        ...
        <span v-if="node.data.host && !node.data.useSSH">{{ node.data.host }}</span>
        <span v-if="node.data.useSSH">{{ node.data.sshHost }}</span>
      </template>
    </tree>
    <context-menu ref="serverContextMenu" :items="serverMenuItems"></context-menu>
    <context-menu ref="dbContextMenu" :items="dbMenuItems"></context-menu>
    <context-menu ref="filterContextMenu" :items="filterMenuItems"></context-menu>
    <context-menu ref="panelContextMenu" :items="panelMenuItems"></context-menu>
    <new-redis-server-modal @created="serverCreated" @updated="serverUpdated" ref="newRedisServerModal"></new-redis-server-modal>
    <new-keys-filter-modal @newKeysFilter="afterNewKeysFilter" ref="newKeysFilterModal"></new-keys-filter-modal>
    <b-modal ref="deleteConfirmModal" size="sm" :title="$t('deleteRedisServerConfirmModalTitle').toString()" @ok="deleteRedisServer">
      <div class="row">
        <div class="col-3">
    ...
    <b-modal ref="deleteKeysFilterConfirmModal" size="sm" :title="$t('deleteKeysFilterConfirmModalTitle').toString()" @ok="deleteKeysFilter">
      <div class="row">
        <div class="col-3">
          <i class="fa fa-4x text-danger fa-exclamation-triangle"></i>
        </div>
...
export default class Left extends Vue {
   ...
   */
  private serverMenuItems: ContextMenuItem[] = [new ContextMenuItem("fa fa-plus", i18n.t("newRedisConnection").toString(), this.showNewServerModal), new ContextMenuItem("fa fa-minus", i18n.t("deleteRedisConnection").toString(), this.showDeleteServerModal), new ContextMenuItem("fa fa-cog", i18n.t("editRedisConnection").toString(), this.showEditServerModal), new ContextMenuItem("fa fa-file-export", i18n.t("exportServerList").toString(), this.showExportModal), new ContextMenuItem("fa fa-file-import", i18n.t("importServerList").toString(), this.showImportModal)]

  /**
   * When right click database node, show these menu items
   */
  private dbMenuItems: ContextMenuItem[] = [new ContextMenuItem("fa fa-plus", i18n.t("newFilterPrefix").toString(), this.showNewFilterModal), new ContextMenuItem("fa fa-file-export", i18n.t("exportServerList").toString(), this.showExportModal), new ContextMenuItem("fa fa-file-import", i18n.t("importServerList").toString(), this.showImportModal)]

  /**
   * When right click filter node, show these menu items
   */
  private filterMenuItems: ContextMenuItem[] = [new ContextMenuItem("fa fa-minus", i18n.t("deleteFilterPrefix").toString(), this.showDeleteFilterConfirmModal), new ContextMenuItem("fa fa-file-export", i18n.t("exportServerList").toString(), this.showExportModal), new ContextMenuItem("fa fa-file-import", i18n.t("importServerList").toString(), this.showImportModal)]

  /**
   * When right click left panel, show these menu items
   */
  private panelMenuItems: ContextMenuItem[] = [new ContextMenuItem("fa fa-plus", i18n.t("newRedisConnection").toString(), this.showNewServerModal), new ContextMenuItem("fa fa-file-export", i18n.t("exportServerList").toString(), this.showExportModal), new ContextMenuItem("fa fa-file-import", i18n.t("importServerList").toString(), this.showImportModal)]
  ...
  private deleteKeysFilter(): void {
    if (this.selectedFilterNode) {
      ...
      Store.removeKeysFilter(this.selectedFilterNode.data.server.name, this.selectedFilterNode.data.index, this.selectedFilterNode.title)
    }
  }
  ...
  private showServerContextMenu(node: ISlTreeNode<any>, event: MouseEvent): void {
      ...
      if (n.model.children.length === 0 || forceLoad) {
        ...
        RedisStore.databases(node.data).then(size => {
          for (let i = 0; i < size; i++) {
            const keysFilters: KeysFilter[] = Store.getKeysFilter(node.data.name, i)
